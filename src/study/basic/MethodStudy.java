 ...
 *
 *
 * - 메소드를 왜? 사용할까?? -> 같은 작업들을 여러 곳에서 자주 사용하게 될 경우 메소드로 따로 작업해두면 해당 메소드만 콜하면
 * 되지만, 그렇지 않은 경우 같은 코드를 여러군데에서 복사 & 붙여넣기 하게 된다! 이때 해당 작업에 수정사항이 생겼을 경우 메소드로 짜둔
 * 경우 메소드만 수정하면 전체에 적용되지만, 그렇지 않은 경우 복붙한 모든 코드를 수정해줘야 하므로 유지 보수에 시간과 비용이 많이
 * 들어간다! 때문에 메소드로 사용하는것이 훨씬 관리하기 편리하다!
 *
